 ...
 *
 */
public class PdrcBsmDispatchComparator implements Comparator<PdrcBsmDispatch> {

	/**
	 * 先按员工号排序，再按月份排序
	 */
	@Override
	public int compare(PdrcBsmDispatch o1, PdrcBsmDispatch o2) {
		...
		String dispatchMonth1 = o1.getDispatchMonth();
		String dispatchMonth2 = o2.getDispatchMonth();
		...
		// return -1;
		if (DateUtil.compareTwoDate(dispatchMonth1, dispatchMonth2)) {
			return -1;
		} else if (DateUtil.compareTwoDate(dispatchMonth2, dispatchMonth1)) {
			return 1;
		} else if (projectID2.compareTo(projectID1) > 0) {
			return 1;
		} else if (projectID1.compareTo(projectID2) > 0) {
			return -1;
		} else if (work1 < work2) {
